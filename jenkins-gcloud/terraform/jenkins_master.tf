...
resource "google_compute_firewall" "allow_access_to_grafana_ui" {
  project = var.project
  name    = "allow-access-to-grafana-ui"
  network = google_compute_network.management.self_link
...
// grafana lb
resource "google_compute_http_health_check" "grafana_health_check" {
...
resource "google_compute_forwarding_rule" "grafana_forwarding_rule" {
